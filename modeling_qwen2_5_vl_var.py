...
class Qwen2_5_VLAttention(nn.Module):
    ...
    def forward(
        ...
        # >>> VAR: Head Fork - identify heads needing modification
        if HeadFork._flag():
            HeadFork.run_logic(attn_weights, self.layer_idx)

        # >>> VAR: Attention Redistribution - modify attention weights
...
class Qwen2_5_VLDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # >>> VAR: Dim Prospector - detect sink tokens (only during prefill, skip first 2 layers)
        if DimProspector._flag() and MetadataStation.get_output_token_count() < 0:
            layer_idx = self.self_attn.layer_idx
            if layer_idx >= 2:  # Skip first 2 layers as per original paper
