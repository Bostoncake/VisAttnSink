...
class HFQwen25VLRunner:
    ...
    def generate(self, prompt: str, images: List[Image.Image], **gen_kwargs: Any) -> str:
        ...
        # VAR: Activate and configure if using var method
        if gen_method == "var":
            # Import VAR components from modeling file
            from modeling_qwen2_5_vl_var import LogicEngine, MetadataStation, StashEngine

            # Export model config
            StashEngine.export_model_config(self.model.config)

            # Activate VAR with parameters
            tau = gen_method_kwargs.get("threshold", 20)
            p = gen_method_kwargs.get("attn_portion", 0.6)
            rho = gen_method_kwargs.get("selected_heads", 0.5)  # Default from paper is 0.5
            summ = 0.2  # Hardcoded, not exposed in evaluate.py args
            except_last_layer = True  # Standard setting
            ...
            LogicEngine.activate(
                ...
                summ=summ,
                p=p,
                except_last_layer=except_last_layer,
                dim_indices=None,  # Will use model defaults or raise error
        ...
        # VAR: Track image token positions
        if gen_method == "var":
        ...
        with torch.no_grad():       # note that this does not affect SLOT implementations
            if gen_method == "adapt_vis":
                ...
                # VAR method - uses modified attention from modeling_qwen2_5_vl_var
                output_ids = self.model.generate(
                    **inputs,
                    max_new_tokens=max_new_tokens,
                    do_sample=do_sample,
        ...
        # VAR: Clear state after generation
        if gen_method == "var":
            LogicEngine.clear()
